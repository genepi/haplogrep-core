...
public class ContaminationTests {
	...
	/*
	 * @Test public void testContaminated1000G() throws Exception {
	 * 
	 * Phylotree phylotree =
	 * PhylotreeManager.getInstance().getPhylotree("phylotree17.xml",
	 * "weights17.txt"); String folder =
	 * "test-data/contamination/1000G/contaminated/"; String variantFile = folder +
	 * "contaminated.vcf.gz"; String out = folder + "contaminated-report.txt";
	 * 
	 * VcfImporter reader2 = new VcfImporter(); HashMap<String, Sample>
	 * mutationServerSamples = reader2.load(new File(variantFile), false);
	 * 
	 * VariantSplitter splitter = new VariantSplitter();
	 * 
	 * ArrayList<String> profiles = splitter.split(mutationServerSamples);
	 ...
	 * String[] splits = profiles.get(0).split("\t");
	 * 
	 * for (int i = 3; i < splits.length; i++) { set.add(splits[i]); }
	 * 
	 * ContaminationDetection contamination = new ContaminationDetection();
	 * 
	 * HaplogroupClassifier classifier = new HaplogroupClassifier(); SampleFile
	 * haplogrepSamples = classifier.calculateHaplogrops(phylotree, profiles);
	 * 
	 * contamination.setSettingAmountHigh(2); contamination.setSettingAmountLow(1);
	 * contamination.setSettingHgQuality(0.5);
	 * 
	 * ArrayList<ContaminationObject> list =
	 * contamination.detect(mutationServerSamples,
	 * haplogrepSamples.getTestSamples()); contamination.writeFile(list, out);
	 * 
	 * CsvTableReader readerOut = new CsvTableReader(out, '\t'); int countHigh = 0;
	 * int countLow = 0; while (readerOut.next()) {
	 * 
	 * if (readerOut.getString("Contamination").equals(Status.HIGH.name())) {
	 * countHigh++; }
	 * 
	 ...
	 * countLow++; } }
	 * 
	 * assertEquals(127, countHigh); //assertEquals(81, countLow);
	 * 
	 * FileUtil.deleteFile(out);
	 * 
	 * }
