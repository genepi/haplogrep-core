...
public class Polymorphism implements Comparable<Polymorphism>, Serializable {
	 ...
	 */
	Reference reference ;// Reference.getInstance();
	 ...
	 */
	Polymorphism(int newPosition, Mutations mutatedBase, Reference ref) {
		this.reference=ref;
	...
	Polymorphism(int newPosition, Mutations mutatedBase, int isReliable, Reference ref) {
		this.reference=ref;
		this.mutation = mutatedBase;
		this.position = newPosition;
		hashCode = toString().hashCode();
	 ...
	 */
	public Polymorphism(String phyloString, Reference ref) throws InvalidPolymorphismException {
		this.reference=ref;
		parse(phyloString);
		hashCode = toString().hashCode();
	...
	public Polymorphism(String phyloString, int isReliable, Reference ref) throws InvalidPolymorphismException {
		this.reference=ref;
		parse(phyloString);
		hashCode = toString().hashCode();
		this.setHeteroplasmy(false);
	...
	public Polymorphism(String phyloString, boolean Heteroplasmy, Reference ref) throws InvalidPolymorphismException {
		this.reference=ref;
		parse(phyloString);
	...
	private Polymorphism getReferenceBase(int position) {
		if (position < reference.getLength() && position > 0) {
			...
			try {
				return new Polymorphism(position, Mutations.getBase(base), reference);
	...
	/*public static String getReferenceBaseSingle1(int position) throws InvalidBaseException {
		...
		// take it as it is.
			...
			if (m.find()) {
				...
				if (stringToParse.trim().length() > 0) {
					...
					try {
						getTransitionPoly(position, reference);
					} catch (Exception e) {
						// TODO Auto-generated catch block
						position = 0;
						e.printStackTrace();
	...
	private void getTransitionPoly(int position, Reference ref) throws InvalidBaseException {
	...
	public boolean isMTHotspot() {
		...
		try {
			if (this.equals(new Polymorphism("315.1C", reference)))
				return true;
			if (this.equals(new Polymorphism("309.1C", reference)))
				return true;
			if (this.equals(new Polymorphism("309.1CC", reference)))
				return true;
			if (this.equals(new Polymorphism("523d", reference)))
				return true;
			if (this.equals(new Polymorphism("524d", reference)))
				return true;
			if (this.equals(new Polymorphism("524.1AC", reference)))
				return true;
			if (this.equals(new Polymorphism("524.1ACAC", reference)))
				return true;
			if (this.equals(new Polymorphism("3107d", reference)))
				return true;
			if (this.equals(new Polymorphism("16182C", reference)))
				return true;
			if (this.equals(new Polymorphism("16183C", reference)))
				return true;
			if (this.equals(new Polymorphism("16193.1C", reference)))
				return true;
			if (this.equals(new Polymorphism("16193.1CC", reference)))
				return true;
			if (this.equals(new Polymorphism("16519", reference)))
				return true;
			else
				return false;
		} catch (NumberFormatException e) {
