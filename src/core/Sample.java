...
public class Sample {
	...
	public Sample(ArrayList<Polymorphism> polymorphisms, Reference reference, SampleRanges sampleRange) {
	 ...
	 * @return 1 if the polymorphism appears in this sample, 2 if heteroplasmy 0
	 *         otherwise
	 */
		...
		for (Polymorphism currentPoly : sample) {
			if (currentPoly.isHeteroplasmy) {
				if (currentPoly.getPosition() == polyToCheck.getPosition()) {
					...
					if ((currentPoly.getMutation().toString().equals("Y")
							&& (polyToCheck.getMutation().toString().equals("C") || polyToCheck.getMutation().toString().equals("T")))
							|| (currentPoly.getMutation().toString().equals("R")
									&& (polyToCheck.getMutation().toString().equals("A") || polyToCheck.getMutation().toString().equals("G")))) {
						return 2;
					}
				}
		...
		for (String currentPoly : sample) {
			...
			if (!currentPoly.contains("5899.1d!") && !currentPoly.contains("65.1T(T)")) {
				...
				if (currentPoly.contains(".")) {
					...
					if (callMethod == 0) {
						...
						if (currentPoly.contains(".1") && currentPoly.matches("\\d+\\.\\d\\w")) {// means:
							...
							if (isReliable == 0) {
								filteredSample.add(new Polymorphism(reference, newInsert + buffer.toString()));
							} else {
								filteredSample.add(new Polymorphism(reference, newInsert + buffer.toString(), 1));
							}
						}
						...
						// is
							...
							Polymorphism newPoly = new Polymorphism(reference, currentPoly);
				...
				// HETEROPLASMY - split in Bases
					...
					Polymorphism newPoly = new Polymorphism(reference, currentPoly, true);
					filteredSample.add(newPoly);
				...
				// Resolve deletation ranges e.g. 1800-1804d
					...
					for (int i = startPosition; i <= endPosition; i++) {
						...
						if (isReliable == 0)
							filteredSample.add(new Polymorphism(reference, i, Mutations.DEL));
						else
							filteredSample.add(new Polymorphism(reference, i, Mutations.DEL, 1));
						startPosition++;
					}
				} else {
					Polymorphism newPoly;
					if (isReliable == 0)
						newPoly = new Polymorphism(reference, currentPoly);
					else
						newPoly = new Polymorphism(reference, currentPoly, 1);
					filteredSample.add(newPoly);
				}
			}
		}
