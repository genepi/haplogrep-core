...
public class OverviewTreeLeafNode extends TreeNode {
	...
	public void updatePolys(boolean includeHotspots, Reference reference) {
		...
		/*
		 * while(c != null){ foundPolysAllSamples.addAll(c.getExpectedPoly()); c
		 * = (OverviewTreeInnerNode) c.getParent(); }
		 */
...
//		testSample.getResults().get(0).getSearchResult().getDetailedResult().getFoundNotFoundPolys()
		...
		for (Polymorphism currentSamplePoly : testSample.getResult(testSample.getExpectedHaplogroup()).getSearchResult().getDetailedResult()
				.getExpectedPolys()) {
...
//			if( !testSample.getResults().get(0).getSearchResult().getDetailedResult().getExpectedPolys().contains(newBackmutation))
		 ...
		 * if(!currentSamplePoly.isBackMutation() &&
		 * !testSample.getSample().getPolymorphisms().contains(currentSamplePoly
		 * )){ Polymorphism newBackmutation = new
		 * Polymorphism(currentSamplePoly);
		 * 
		 * newBackmutation.setBackMutation(true); if(
		 * !testSample.getResults().get(0).getSearchResult().getDetailedResult()
		 * .getExpectedPolys().contains(newBackmutation))
		 * remainingPolys.add(newBackmutation); } }
		 * 
		 * // if(includeMissingPolys) for(Polymorphism currentExpectedPoly :
		 * testSample.getResults().get(0)
		 * .getSearchResult().getDetailedResult().getExpectedPolys()){
		 * if(!foundPolysAllSamples.contains(currentExpectedPoly)){ Polymorphism
		 * newBackmutation = new Polymorphism(currentExpectedPoly);
		 * newBackmutation.setBackMutation(true);
		 * remainingPolys.add(newBackmutation); } }
		 */
